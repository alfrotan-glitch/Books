...
- **ورق اول — حاجی نورالله**، ۶۴ ساله، دیابت دارد. Creatinine او از دو روز قبل «کمی» بلند رفته است. نرس می‌گوید ادرارش امروز خیلی کم بوده. او دیروز برای CT با کنتراست رفته بود.
...
## Creatinine و eGFR — خواندن درست پنجرهٔ وظیفه
...
⚠️ **Creatinine یک «ترمامتر دیرکار» است:** وقتی GFR ناگهان سقوط می‌کند، Creatinine چند روز وقت می‌گیرد تا به سطح جدید برسد. در این فاصله، eGFR **قابل اعتماد نیست**.

### ۲.۲ eGFR چیست و کدام معادله؟

eGFR عددی است که لابراتوار از Creatinine، سن و جنس با یک معادله حساب می‌کند.
...
💡 «**eGFR فقط وقتی راست می‌گوید که Creatinine آرام نشسته باشد.**»

---

## تصنیف CKD با KDIGO 2024 — سیستم CGA
...
**نقشهٔ حرارتی (Heat map):** ترکیب G و A خطر عدم کفایهٔ گرده، امراض قلبی-عروقی و مرگ را در چهار رنگ نشان می‌دهد: سبز (کم)، زرد (متوسط)، نارنجی (بلند)، سرخ (خیلی بلند). یک مریض با G2A3 می‌تواند خطری بیشتر از G3aA1 داشته باشد — **Albuminuria وزن زیاد دارد**.
...
| قصه | اسهال، سپسیس، دوای تازه، کنتراست، انسداد | دیابت و فشار خون طولانی |
...
💡 «**لوله، فلتر، فاضلاب**» — Prerenal = لولهٔ آب، Intrinsic = خود فلتر، Postrenal = فاضلاب بند.

---

## معاینهٔ ادرار (Urinalysis) — ارزان‌ترین بیوپسی گرده

### ۵.۱ جمع‌آوری درست

- ادرار **تازه**، **وسط جریان (Midstream clean-catch)**؛ هرچه زودتر معاینه شود (حجرات و Casts با گذشت وقت خراب می‌شوند).
- حیض، تمرین شدید، مقاربت و کتتر تازه می‌توانند خون یا پروتین کاذب بدهند؛ در صورت امکان وقت معاینه را تغییر بده.
...
| **Glucose** | گلوکوز ادرار | در دیابت کنترول‌نشده یا مریض تحت **SGLT2 inhibitor** (اثر دوا، نه خرابی) |
...
💡 **تشبیه:** Cast یک **قالب** از داخل توبول است — مثل اینکه در یک لوله گچ بریزی و بعد بیرون بکشی. هرچه در آن قالب گیر مانده باشد (RBC، WBC، حجرهٔ مرده)، نشان می‌دهد در **داخل گرده** چه می‌گذرد.

---

## Proteinuria — چقدر و از کجا؟
...
- **دامنهٔ نفروتیک:** پروتین ادرار **بیشتر از ۳.۵ g/روز**. **سندروم نفروتیک** = این مقدار **به اضافهٔ Albumin سیروم کمتر از ۳۰ g/L**، با یا بدون ادیما (KDIGO 2021 Glomerular Diseases). ← احمد شاه را به فصل ۳۷ ببر.
- **سندروم نفریتیک:** Haematuria گلومرولی (RBC casts/Dysmorphic RBC) + پروتین + غالباً فشار بلند، ادیما و کاهش GFR.
...
## Haematuria — واقعی؟ گلومرولی؟ خطر سرطان؟
...
| ۲ | علت گذرا یا خوش‌خیم (حیض، UTI، تمرین شدید، ضربهٔ خفیف)؟ | پس از رفع علت، **Urinalysis را تکرار کن** (AUA) |
...
**عوامل خطر اضافی Urothelial cancer (لست رهنمود، نه محدود به این‌ها):** اعراض تخریشی مجاری ادرار پایین · رادیوتراپی (Radiotherapy) قبلی لگن · سابقهٔ کیموتراپی با Cyclophosphamide/Ifosfamide · سابقهٔ فامیلی Urothelial carcinoma یا Lynch syndrome · تماس شغلی با Benzene یا Aromatic amines · جسم خارجی دوامدار در مجاری ادرار.
...
| **بلند** | **Cystoscopy + تصویربرداری مقطعی مجاری فوقانی** (۱۵): CT Urography چندمرحله‌ای؛ اگر مانع داشت MR Urography؛ اگر هر دو مانع داشتند Retrograde pyelography با تصویربرداری بدون کنتراست یا Ultrasound |
...
💡 ** — ملالی:** dipstick +۲ ← میکروسکوپ. اگر ≥۳ RBC/HPF و علت گذرا نبود ← خطر او را بسنج. زن جوان، غیرسگرتی، بدون عوامل دیگر ← احتمالاً فقط تکرار Urinalysis.
...
| CT بدون کنتراست | سنگ (فصل ۴۱) | — |
| CT Urography | Haematuria پرخطر | کنتراست — نگاه کن به §۹ |
| Doppler شریان گرده | شک به تنگی شریان گرده | — |
| **سیرولوژی** (ANA، anti-dsDNA، C3/C4، ANCA، anti-GBM، anti-PLA2R، الکتروفورز پروتین و Free light chains) | نفریتیک/نفروتیک یا AKI بی‌دلیل | فصل ۳۷ |
| **بیوپسی گرده** | علت نامعلوم با پروتین قابل‌توجه، نفریتیک، AKI بی‌دلیل | تصمیم متخصص؛ فصل ۳۷ |

---

## دواها و گرده — پیشگیری از آسیب

### ۹.۱ کنتراست یودین‌دار داخل وریدی (ACR–NKF Consensus 2020)

- خطر واقعی AKI ناشی از کنتراست **نامعلوم** است، اما در مرض شدید گرده احتیاط لازم است.
...
- **زمان، حجم و سرعت مطلوب Normal saline نامعلوم است** (خود بیانیه)؛ از این رو این فصل حجم ثابتی نمی‌دهد. پروتوکول محلی و وضعیت حجم مریض تعیین‌کننده است.
- ⚠️ **تفاوت منابع:** KDIGO 2012 (AKI) آستانهٔ وقایه را **eGFR کمتر از ۴۵** گذاشته بود؛ ACR–NKF 2020 آستانه را **کمتر از ۳۰** می‌داند. منبع جدیدتر و مخصوص کنتراست داخل وریدی = ACR–NKF 2020.
...
| قدم | کار | چرا |
|---|---|---|
| ۱ | **ABCDE و خطرهای فوری:** پوتاشیم، اسیدوز، ادیمای شش، Uraemia (Pericarditis، اختلال شعور)، Anuria | این‌ها «اندیکاسیون‌های عاجل دیالیز» را می‌سازند (فصل ۳۸) |
| ۲ | **Creatinine قبلی را پیدا کن** | AKI، CKD یا هر دو |
| ۳ | **حجم ادرار و وضعیت حجم** | کم‌آبی یا اضافه‌بار |
| ۴ | **لست دواها** (NSAID، ACEi/ARB، Diuretic، Metformin، کنتراست تازه، انتی‌بیوتیک‌ها) | علت قابل برگشت |
...
- **عاجل:** AKI شدید یا با پوتاشیم بلند مقاوم، ادیمای شش، اسیدوز شدید؛ **RBC casts یا شک به GN تیزرفتار**؛ AKI بی‌دلیل.
...
- ⚠️ **Anuria ناگهانی** — انسداد یا حادثهٔ عروقی را فوراً رد کن (Ultrasound).
- ⚠️ **RBC casts + Creatinine رو به بالا** — GN تیزرفتار؛ همان روز متخصص.
- ⚠️ **Haematuria + خونریزی ریوی (Haemoptysis)** — سندروم ریوی-گرده‌ای (anti-GBM، ANCA vasculitis).
- ⚠️ **dipstick خون مثبت بدون RBC + درد عضله/ادرار تاریک** — Rhabdomyolysis.
- ⚠️ **ادیمای شش، اسیدوز شدید، علایم Uraemia** — اندیکاسیون احتمالی دیالیز عاجل.
...
5. پیش از CT با کنتراست، **eGFR تازه** را ببین.
...
4. وقایهٔ کنتراست داخل وریدی برای کی؟ — *AKI یا eGFR کمتر از ۳۰ بدون دیالیز (ACR–NKF 2020).*
